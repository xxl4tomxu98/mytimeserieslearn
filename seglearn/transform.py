...
__all__ = ['Segment', 'SegmentX', 'SegmentXY', 'SegmentXYForecast',
           'PadTrunc', 'InterpLongToWide', 'Interp', 'FeatureRep',
...
class Segment(BaseEstimator, XyTransformerMixin):
    ...
    overlap : float range [0,1]
        ...
        (note: setting overlap to 1.0 results in the segments to being
         advanced by a single sample)
    step : int range [1, width] (default=None)
        number of samples to advance adjacent segments (note: this takes
        precedence over overlap)
    y_func : function
        returns target from array of target segments (eg ``last``, ``middle``,
        or ``mean``)
    shuffle : bool, optional
        shuffle the segments after transform (recommended for batch
        optimizations)
    random_state : int, default = None
        Randomized segment shuffling will return different results for each
        call to ``transform``. If you have set ``shuffle`` to True and want
        the same result with each call to ``fit``, set ``random_state``
        to an integer.
    order : str, optional (default='F')
        Determines the index order of the segmented time series. 'C' means
        C-like index order (first index changes slowest) and 'F' means
        Fortran-like index order (last index changes slowest). 'C' ordering
        is suggested for neural network estimators, and 'F' ordering is
        suggested for computing feature representations.
    Returns
    -------
    self : object
        Returns self.
    """
    def __init__(self, width=100, overlap=0.5, step=None, y_func=last,
                 shuffle=False, random_state=None, order='F'):
        self.width = width
    ...
    def _validate_params(self):
        ...
        if self.overlap is not None and not (0.0 <= self.overlap <= 1.0):
            raise ValueError("overlap must be >=0 and <=1.0 (was %.2f)"
                             % self.overlap)
        if self.step is not None and not (1 <= self.step <= self.width):
            raise ValueError('step must be >=1 and <=width=%s (was %s)'
                             % (self.width, self.step))
        if self.overlap is None and self.step is None:
            raise ValueError('Either overlap or step must be set to a\
                              valid number')
        if self.order not in ('C', 'F'):
            raise ValueError('order must be either "C" or "F" (was %s'
                             % self.order)
    ...
    def transform(self, X, y=None, sample_weight=None):
        """
        Transforms the time series data into segments (temporal tensor). Note
        this transformation changes the number of samples in the data, If y
        and sample_weight are provided, they are transformed to align to the
        new samples
        Parameters
        ----------
        X : array-like, shape [n_series, ...]
    ...
    def _segmentX(self, X):
        ...
        if Xt[0].ndim > 1:
            Xt = np.array(
                [sliding_tensor(Xt[i], self.width, self._step, self.order)
                 for i in np.arange(N)], dtype=object
                 )
        else:
            Xt = np.array(
                [sliding_window(Xt[i], self.width, self._step, self.order)
                 for i in np.arange(N)], dtype=object
                 )
    ...
    def _segmentY(self, y, Nt, ts_target=False):
        if ts_target:
            yt = np.array(
                [sliding_window(y[i], self.width, self._step, self.order)
...
class SegmentX(Segment):
    ...
    random_state : int, default = None
        Randomized segment shuffling will return different results for each
        call to ``transform``. If you have set ``shuffle`` to True and want the
        same result with each call to ``fit``, set ``random_state`` to an int.
    order : str, optional (default='F')
        Determines the index order of the segmented time series. 'C' means
        C-like index order (first index changes slowest) and 'F' means
        Fortran-like index order (last index changes slowest). 'C' ordering is
        suggested for neural network estimators, and 'F' ordering is suggested
    ...
    """
    def __init__(self, width=100, overlap=0.5, step=None, shuffle=False,
                 random_state=None, order='F'):

        super().__init__(width=width, overlap=overlap, step=step,
...
class SegmentXYForecast(Segment):
    ...
    The target y is mapped to segments from their parent series,
    using the ``forecast`` and ``y_func`` parameters to determine the mapping
    behavior. The segment targets can be a single value, or a sequence of
    values depending on ``y_func`` parameter.
    The transformed data consists of segment/target pairs that can be learned
    through a feature representation or directly with a neural network.
    Parameters
    ----------
    ...
    overlap : float range [0,1]
        amount of overlap between segments. must be in range: 0 <= overlap <= 1
        (note: setting overlap to 1.0 results in the segments to being advanced
         by a single sample)
    step : int range [1, width] (default=None)
        number of samples to advance adjacent segments (note: this takes
        precedence over overlap)
    ...
    y_func : function
        returns target from array of target forecast segments (eg ``last``,
        or ``mean``)
    shuffle : bool, optional
        shuffle the segments after transform (recommended for batch
        optimizations)
    random_state : int, default = None
        Randomized segment shuffling will return different results for each
        call to ``transform``. If you have set ``shuffle`` to True and want
        the same result with each call to ``fit``, set ``random_state`` to
        an integer.
    order : str, optional (default='F')
        Determines the index order of the segmented time series. 'C' means
        C-like index order (first index changes slowest) and 'F' means
    ...
    self : object
        Returns self.
    """
    def __init__(self, width=100, overlap=0.5, step=None, forecast=10,
                 y_func=last, shuffle=False, random_state=None, order='F'):

        super().__init__(width=width, overlap=overlap,
    ...
    def transform(self, X, y=None, sample_weight=None):
        """
        Forecast sliding window segmentation for time series or sequence
        datasets. Note this transformation changes the number of samples
        in the data. Currently sample weights always returned as None.
        Parameters
        ----------
        X : array-like, shape [n_series, ...]
           Time series data and (optionally) contextual data
        ...
        N = len(Xt)  # number of time series
        if Xt[0].ndim > 1:
            Xt = np.array([sliding_tensor(Xt[i], self.width + self.forecast,
                                          self._step, self.order)
                           for i in np.arange(N)], dtype=object)
        else:
            Xt = np.array([sliding_window(Xt[i], self.width + self.forecast,
                                          self._step, self.order)
                           for i in np.arange(N)], dtype=object)
        Nt = [len(Xt[i]) for i in np.arange(len(Xt))]
        Xt = np.concatenate(Xt).astype(float)
        ...
        if Xc is not None:
            ...
            Xt = TS_Data(Xt, Xc)
        if yt is not None:
            yt = np.array([sliding_window(yt[i], self.width + self.forecast,
                                          self._step, self.order)
                           for i in np.arange(N)], dtype=object)
            yt = np.concatenate(yt).astype(float)
...
def expand_variables_to_segments(v, Nt):
    """ expands contextual variables v, by repeating each instance as
        specified in Nt """
    N_v = len(np.atleast_1d(v[0]))
    return np.concatenate([np.full((Nt[i], N_v), v[i])
                           for i in np.arange(len(v))], axis=0)
...
def sliding_window(time_series, width, step, order='F'):
    ...
    """
    w = np.hstack([time_series[i:1 + i - width or None:step]
...
def sliding_tensor(mv_time_series, width, step, order='F'):
    ...
    data = [sliding_window(mv_time_series[:, j], width, step, order)
            for j in range(D)]
...
class PadTrunc(BaseEstimator, XyTransformerMixin):
    """
    Transformer for using padding and truncation to enforce fixed length on
    all time series in the dataset. Series' longer than ``width`` are truncated
    to length ``width``. Series' shorter than length ``width`` are padded at
    the end with zeros up to length ``width``.

    The same behavior is applied to the target if it is a series and passed to
    the transformer.
    ...
    def fit(self, X, y=None):
        ...
        X : array-like, shape [n_series, ...]
            Time series data and (optionally) contextual data
        y : None
            There is no need of a target in a transformer, yet the
    ...
    def transform(self, X, y=None, sample_weight=None):
        """
        Transforms the time series data into fixed length segments using
        padding and or truncation. If y is a time series and passed,
        it will be transformed as well.
        Parameters
        ----------
        X : array-like, shape [n_series, ...]
           Time series data and (optionally) contextual data
...
class Interp(BaseEstimator, XyTransformerMixin):
    """
    Transformer for resampling time series data to a fixed period over closed
    interval (direct value interpolation). Default interpolation is linear,
    but other types can be specified. If the target is a series,
    it will be resampled as well.
    ...
    If start time or similar is important to the estimator, use a context
    variable.
    ...
    categorical_target : bool
        set to True for classification problems to use nearest instead of
        linear interp for the target
    assume_sorted : bool
        assume time series data is sorted by timestamp
    """
    def __init__(self, sample_period, kind='linear',
                 categorical_target=False, assume_sorted=True):
        if not sample_period > 0:
            raise ValueError("sample_period must be >0 (was %f)"
                             % sample_period)
    ...
    def _interp(self, t_new, t, x, kind):
        if not self.assume_sorted:
            t, x = interp_sort(t, x)
        interpolator = interp1d(t, x, kind=kind, copy=False,
                                bounds_error=False,
    ...
    def transform(self, X, y=None, sample_weight=None):
        ...
        sample_weight_new : array-like or None
            None is returned if target is changed. Otherwise it is returned
            unchanged.
        """
        ts_target = check_ts_data(X, y)
        Xt, Xc = get_ts_data_parts(X)
        ...
        t_lin = [np.arange(Xt[i][0, 0], Xt[i][-1, 0], self.sample_period)
                 for i in np.arange(N)]
        if D == 1:
            Xt = [self._interp(t_lin[i], t[i], Xt[i][:, 1], kind=self.kind)
                  for i in np.arange(N)]
        elif D > 1:
            Xt = [np.column_stack([self._interp(t_lin[i], t[i], Xt[i][:, j],
                                   kind=self.kind) for j in range(1, D + 1)])
                  for i in np.arange(N)]
        if Xc is not None:
            Xt = TS_Data(Xt, Xc)
        if yt is not None and ts_target:
            ...
            if self.categorical_target is True:
                yt = [self._interp(t_lin[i], t[i], yt[i], kind='nearest')
                      for i in np.arange(N)]
            else:
                yt = [self._interp(t_lin[i], t[i], yt[i], kind=self.kind)
                      for i in np.arange(N)]
        else:
...
class InterpLongToWide(BaseEstimator, XyTransformerMixin):
    """
    Converts time series in long format dataframes (where variables are
    sampled at different times) to wide format data frames usable by the
    rest of seglearn using direct value interpolation.

    Input data for this class must have at least 3 columns of type (time,
    var_type, var_value) Additional columns are treated as additional
    channels of var_value (e.g. time, var_type, var_value1, var_value2).

    Each time series must have the same var_types and the same number of
    columns.
    ...
    The interpolation to a linear sampling space, and conversion to wide
    format dataframe results in the removal of the time column and var_type
    columns in the data.
    ...
    >>>
    >>> # sample stacked input with values from 2 variables each with
        # 2 channels
    ...
    def _check_data(self, X):
        """
        Checks that unique identifiers vaf_types are consistent between time
        series.
        Parameters
        ----------
        X : array-like, shape [n_series, ...]
        ...
        if len(X) > 1:
            ...
            if np.all([np.all(np.unique(X[i][:, 1]) == sval)
                      for i in range(1, len(X))]):
                pass
            else:
                raise ValueError(
                    "Unique identifier var_types not \
                     consistent between time series")
    ...
    def transform(self, X, y=None, sample_weight=None):
        ...
        for i in np.arange(N):
            ...
            # find latest/earliest sample time for each identifier's
            # first/last time sample time
            t_min = np.max([np.min(xs[j][:, 0]) for j in np.arange(len(s))])
            t_max = np.min([np.max(xs[j][:, 0]) for j in np.arange(len(s))])
            # Generate a regular series of timestamps starting at tStart and
            # tEnd for sample_period
            t_lin.append(np.arange(t_min, t_max, self.sample_period))
            # Interpolate for the new regular sample times
            if d == 1:
                x_new.append(
                    np.column_stack(
                        [self._interp(t_lin[i], xs[j][:, 0], xs[j][:, 2],
                                      kind=self.kind)
                         for j in np.arange(len(s))]))
            elif d > 1:
                xd = []
                for j in np.arange(len(s)):
                    # stack the columns of each variable by dimension d after
                    # interpolation to new regular sample times
                    temp = np.column_stack(
                        [(self._interp(t_lin[i], xs[j][:, 0], xs[j][:, k],
                          kind=self.kind))
                         for k in np.arange(2, 2 + d)])
                    xd.append(temp)
                # column stack each of the sensors s -- resulting in s*d
                # columns
        ...
        if yt is not None and ts_target:
            ...
            swt = None
            if self.categorical_target is True:
                yt = [self._interp(t_lin[i], xt[i][:, 0], yt[i],
                                   kind='nearest') for i in np.arange(N)]
            else:
                yt = [self._interp(t_lin[i], xt[i][:, 0], yt[i],
                                   kind=self.kind) for i in np.arange(N)]
        else:
            # y is static - leave y alone
            pass
        if xc is not None:
...
class FeatureRep(BaseEstimator, TransformerMixin):
    """
    A transformer for calculating a feature representation from segmented time
    series data. This transformer calculates features from the segmented time
    series', by computing the same feature set for each segment from each time
    series in the data set. The ``features`` computed are a parameter of this
    transformer, defined by a dict of functions. The seglearn package includes
    some useful features, but this basic feature set can be easily extended.

    Parameters
    ----------
    features : dict, optional
        Dictionary of functions for calculating features from a segmented time
        series. Each function in the dict is specified to compute features
        from a multivariate segmented time series along axis 1 (the segment):
            >>> def mean(X):
            >>>    F = np.mean(X, axis = 1)
            >>>    return F
        ...
        If features is not specified, a default feature dictionary will be
        used (see base_features). See ``feature_functions`` for example
        implementations.
    ...
    Attributes
    ----------
    f_labels : list of string feature labels (in order) corresponding to
    ...
    def __init__(self, features='default', verbose=False):
        if features == 'default':
            ...
            if not isinstance(features, dict):
                raise TypeError("features must either 'default'\
                                 or an instance of type dict")
    ...
    def transform(self, X):
        ...
        If contextual data is included in X, it is returned with the
        feature data.
        ...
        X_new : array shape [n_series, ...]
            Feature representation of segmented time series data and
            contextual data
        """
    ...
    def _reset(self):
        """ Resets internal data-dependent state of the transformer.
            __init__ parameters not touched. """
        self.f_labels = None
    ...
    def _check_features(self, features, Xti):
        ...
        if not np.all(fshapes[:, 0] == N):
            raise ValueError("feature function returned \
...
class FeatureRepMix(_BaseComposition, TransformerMixin):
    ...
    A transformer for calculating a feature representation from segmented time
    series data. This transformer calculates features from the segmented time
    series', by applying the supplied list of FeatureRep transformers on the
    specified columns of data. Non-specified columns are dropped.

    The segmented time series data is expected to enter this transform in the
    form of num_samples x segment_size x num_features and to leave this
    transform in the form of num_samples x num_features. The term columns
    refers to the last dimension of both representations.

    Note: This code is partially taken (_validate and _transformers functions
          with docstring) from the scikit-learn ColumnTransformer made
          available under the 3-Clause BSD license.

    Parameters
    ----------
    transformers : list of (name, transformer, columns) to be applied on the
                   segmented time series
        name : string
            unique string which is used to prefix the f_labels of the
            FeatureRep below
    ...
    Attributes
    ----------
    f_labels : list of string feature labels (in order) corresponding to the
               computed features
    Examples
    --------
    ...
    def _transformers(self):
        """
        Internal list of transformers only containing the name and
        transformers, dropping the columns. This is for the implementation
        of get_params via BaseComposition._get_params which
    ...
    def get_params(self, deep=True):
        ...
        deep : boolean, optional
            If True, will return the parameters for this transformer and
            contained transformers.
        Returns
        -------
        params : mapping of string to any parameter names mapped to their
                 values.
        """
    ...
    def _retrieve_indices(cols):
        """
        Retrieve a list of indices corresponding to the provided column
        specification.
        """
        if isinstance(cols, int):
            ...
            raise TypeError('No valid column specifier. Only a scalar,\
                             list or slice of all\
    ...
    def _validate(self):
        """
        Internal function to validate the transformer before applying all
        ...
        for trans in transformers:
            if not isinstance(trans, FeatureRep):
                raise TypeError("All transformers must be an instance of\
                                 FeatureRep. '%s' (type %s) doesn't."
                                % (trans, type(trans)))

    def transform(self, X):
        """
        Transform the segmented time series data into feature data.
        If contextual data is included in X, it is returned with the feature
        data.
        Parameters
        ----------
        X : array-like, shape [n_series, ...]
...
class FunctionTransformer(BaseEstimator, TransformerMixin):
    ...
    func : function, optional (default=None)
        the function to be applied to Xt, the time series part of X
        (contextual variables Xc are passed through unaltered) - X
        remains unchanged if no function is supplied
    ...
    def transform(self, X):
        """
        Transforms the time series data based on the provided function. Note
        this transformation must not change the number of samples in the data.
        Parameters
        ----------
        X : array-like, shape [n_samples, ...]
            time series data and (optionally) contextual data
        ...
        if self.func is None:
            ...
            if len(Xt) != n_samples:
                raise ValueError("FunctionTransformer changes \
                                  sample number (not supported).")
            if Xc is not None:
                Xt = TS_Data(Xt, Xc)
            return Xt
